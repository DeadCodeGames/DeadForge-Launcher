...
ipcRenderer.on('connectionCheck', (event, status) => {
  ...
  if (status == false) { setTimeout(document.querySelector('iframe#webiFrame').contentDocument.querySelector('html').classList.add(document.querySelector('html').classList.contains('dark') ? 'dark' : 'light'), 1000) }
})